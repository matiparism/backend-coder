...
class ProductManager {
	...
	addProduct = async (title, description, price, thumbnail, code, stock) => {
		...
		if (!title || !description || !price || !thumbnail || !code || !stock) {
			...
			const repeted_code = this.products.find(
	...
	updateProduct = async (
		...
		if (
			...
			const repeted_code = all_products.find(
			...
			if (repeted_code) {
				...
				await fs.promises.writeFile(
					this.path,
					JSON.stringify(new_products_list, null, 2)
				);
			}
		}
	};
	...
	deleteProduct = async (id) => {
		...
		const products_not_found = all_products.filter(
		...
		await fs.promises.writeFile(
			this.path,
			JSON.stringify(products_not_found, null, 2)
		);
	};
